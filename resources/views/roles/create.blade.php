...
@section('content')
    <form action="{{ route('roles.store') }}" method="POST">
        @csrf
        <div class="card p-4">
            <h2 class="text-center text-uppercase">Crear Nuevo Rol</h2><hr>
            <div class="card-body">
                <div class="row">
                    <div class="col-sm-12">
                        <label for="name" class="form-label">{{ __('Nombre del rol') }}</label>
                        <input type="text" name="name" id="name" value="{{ old('name') }}" class="form-control" placeholder="{{ __('Ingresar el nombre del nuevo rol') }}" required>
